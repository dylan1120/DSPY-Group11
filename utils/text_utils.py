...
def ngrams_count(
    corpus, ngram_range, n=-1, cached_stopwords=stopwords.words("portuguese")
):
    """
    Args
    ----------
    ...
    vectorizer = CountVectorizer(
        stop_words=cached_stopwords, ngram_range=ngram_range
    ).fit(corpus)
...
class StopWordsRemoval(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        return [
            " ".join(stopwords_removal(comment, self.text_stopwords)) for comment in X
        ]
