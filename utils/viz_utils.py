    ...
    for col in features:
        ...
        if hue != False:
            for classe in df[hue].value_counts().index:
                ...
                sns.distplot(
                    df_hue[col], color=color[target_idx], hist=hist, ax=ax, label=classe
                )
                target_idx += 1
        else:
...
def countplot(
    ...
    if hue != False:
        ...
        if order:
            ...
            percent_df.plot(
                kind="bar", stacked=True, ax=axs[1], color=colors, width=width
            )
        else:
            percent_df.plot(
                kind="bar", stacked=True, ax=axs[1], color=colors, width=width
        ...
        # Definindo títulos
        axs[0].set_title(
            f"Análise de Volumetria da Variável {feature}",
            size=14,
            color="dimgrey",
            pad=20,
        )
        ...
        axs[1].set_title(
            f"Análise de Volumetria da Variável {feature} por {hue}",
            size=14,
            color="dimgrey",
            pad=20,
        )
        ...
        # Definindo título
        ax.set_title(
            f"Análise de Volumetria da Variável {feature}", size=14, color="dimgrey"
        )
...
# Função para plotagem de volumetria das variáveis categóricas do conjunto de dados
def numplot_analysis(
    df_numerical,
    fig_cols=3,
    color_sequence=["darkslateblue", "mediumseagreen", "darkslateblue"],
    hue=False,
    color_hue=["darkslateblue", "crimson"],
    hist=False,
):
    """
    Etapas:
    ...
    fig, axs = plt.subplots(
        nrows=fig_rows, ncols=fig_cols, figsize=(fig_cols * 5, fig_rows * 4.5)
    )
    sns.despine(left=True)
    i, j = 0, 0
    ...
    for col in num_features:
        ...
        if hue != False:
            ...
            sns.distplot(
                df_numerical[col], color=color_sequence[color_idx], hist=hist, ax=ax
            )
...
def catplot_percentage_analysis(
    df_categorical, hue, fig_cols=2, palette="viridis", figsize=(16, 10)
):
    """
    Etapas:
        1. retorno das variáveis categóricas do conjunto de dados
    ...
    for col in cat_features:
        ...
        col_to_hue.div(col_to_hue.sum(1).astype(float), axis=0).plot(
            kind="barh", stacked=True, ax=ax, colors=palette
        )
...
def answear_plot(
    ...
    if len(list_cols) != ncols:
        print(
            f"Number of cols passed in list_cols arg is different for figure cols axis. Please check it."
        )
        return None
    ...
    for col in list_cols:
        ...
        sns.barplot(
            x=col, y=grouped_col, data=sorted_data.head(top), ax=ax1, palette=palette
        )
        ax1.set_title(
            f"Top {top} {grouped_col.capitalize()} with Highest \n{col.capitalize()}"
        )
        ...
        if bottom_filter:
            sns.barplot(
                x=col,
                y=grouped_col,
                data=sorted_data[sorted_data[col] > 0].tail(top),
                ax=ax2,
                palette=palette + "_r",
            )
        else:
            sns.barplot(
                x=col,
                y=grouped_col,
                data=sorted_data.tail(top),
                ax=ax2,
                palette=palette + "_r",
            )
        ax2.set_title(
            f"Top {top} {grouped_col.capitalize()} with Lowest \n{col.capitalize()}"
        )
