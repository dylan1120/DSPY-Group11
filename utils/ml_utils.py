...
class BinaryClassifiersAnalysis:
    ...
    def fit(
        ...
        for model_name, model_info in classifiers.items():
            ...
            if random_search:
                ...
                self.classifiers_info[clf_key]["estimator"] = model_info["model"].fit(
                    X, y
                )
    ...
    def compute_train_performance(self, model_name, estimator, X, y, cv=5):
        ...
        try:
            y_scores = cross_val_predict(
                estimator, X, y, cv=cv, method="decision_function"
            )
        except:
        ...
        return (
            pd.DataFrame(train_performance, index=train_performance.keys())
    ...
    def compute_test_performance(self, model_name, estimator, X, y):
        ...
        return (
            pd.DataFrame(test_performance, index=test_performance.keys())
            .reset_index(drop=True)
            .loc[:0, :]
        )
    ...
    def evaluate_performance(
        ...
        for model_name, model_info in self.classifiers_info.items():
            ...
            if "train_performance" in model_info.keys():
                df_performances = df_performances.append(
                    model_info["train_performance"]
                )
            ...
            train_performance = self.compute_train_performance(
                model_name, estimator, X_train, y_train, cv=cv
            )
            test_performance = self.compute_test_performance(
                model_name, estimator, X_test, y_test
            )
        ...
        if save:
            ...
            df_performances = df_performances.loc[
                :, ["anomesdia", "anomesdia_datetime"] + cols_performance
            ]
    ...
    def feature_importance_analysis(
        ...
        if specific_model is not None:
            try:
                model_feature_importance = self.classifiers_info[specific_model][
                    "feature_importances"
                ]
                if graph:
                    ...
                    sns.barplot(
                        x="importance",
                        y="feature",
                        data=model_feature_importance.iloc[:top_n, :],
                        ax=ax,
                        palette=palette,
                    )
                    format_spines(ax, right_border=False)
                    ax.set_title(
                        f"Top {top_n} {specific_model} Features mais Relevantes",
                        size=14,
                        color="dimgrey",
                    )
                ...
                if save:
                    ...
                    model_feature_importance = model_feature_importance.loc[
                        :, order_cols
                    ]
                ...
                print(
                    f"Classificador {specific_model} não existente nas chaves de classificadores treinados."
                )
        ...
        if graph and specific_model is None:
            print(
                "Por favor, escolha um modelo específico para visualizar o gráfico das feature importances"
            )
            return None
    ...
    def plot_roc_curve(self, figsize=(16, 6)):
        ...
        for model_name, model_info in self.classifiers_info.items():
            ...
            plt.subplot(1, 2, 1)
            plt.plot(
                train_fpr, train_tpr, linewidth=2, label=f"{model_name} auc={train_auc}"
            )
            plt.plot([0, 1], [0, 1], "k--")
            ...
            plt.subplot(1, 2, 2)
            plt.plot(
                test_fpr, test_tpr, linewidth=2, label=f"{model_name} auc={test_auc}"
            )
            plt.plot([0, 1], [0, 1], "k--")
            plt.axis([-0.02, 1.02, -0.02, 1.02])
            plt.xlabel("False Positive Rate")
    ...
    def custom_confusion_matrix(
        self, model_name, y_true, y_pred, classes, cmap, normalize=False
    ):
        """
    ...
    def plot_learning_curve(
        ...
        ax.plot(
            train_sizes, train_scores_mean, "o-", color="navy", label="Training Score"
        )
        ax.fill_between(
            train_sizes,
        ...
        ax.plot(
            train_sizes, val_scores_mean, "o-", color="red", label="Cross Val Score"
        )
        ax.fill_between(
            train_sizes,
    ...
    def plot_score_distribution(self, model_name, shade=False):
        ...
        plt.suptitle(
            f"Score Distribution: a Probability Approach for {model_name}\n",
            size=14,
            color="black",
        )
        plt.show()
    ...
    def plot_score_bins(self, model_name, bin_range):
        ...
        bins_labels = [
            str(round(list(bins)[i - 1], 2)) + " a " + str(round(list(bins)[i], 2))
        ...
        for df_scores, ax in zip(
            [df_train_scores, df_test_scores], [axs[0, 0], axs[0, 1]]
        ):
            sns.countplot(
                x="faixa",
                data=df_scores,
        ...
        for df_percent, ax in zip(
            [df_train_percent, df_test_percent], [axs[1, 0], axs[1, 1]]
        ):
            df_percent.plot(
                kind="bar",
                ax=ax,
                stacked=True,
                color=["darkslateblue", "crimson"],
                width=0.6,
            )
        ...
        # Final definitions
        axs[0, 0].set_title(
            "Quantity of each Class by Range - Train", size=12, color="dimgrey"
        )
        axs[0, 1].set_title(
            "Quantity of each Class by Range - Test", size=12, color="dimgrey"
        )
        axs[1, 0].set_title(
            "Percentage of each Class by Range - Train", size=12, color="dimgrey"
        )
        axs[1, 1].set_title(
            "Percentage of each Class by Range - Test", size=12, color="dimgrey"
        )
        plt.suptitle(
            f"Score Distribution by Range - {model_name}\n", size=14, color="black"
        )
        plt.tight_layout()
        plt.show()
...
def cross_val_performance(estimator, X, y, cv=5):
    ...
    df_performance = df_performance.loc[
        :, ["anomesdia", "anomesdia_datetime"] + cols_performance
    ]
...
def elbow_method_kmeans(df, K_min, K_max, figsize=(16, 6)):
    ...
    sns.lineplot(
        x=range(K_min, K_max), y=square_dist, color="cornflowerblue", marker="o"
    )
...
def plot_kmeans_clusters_2d(df, k_means, figsize=(16, 6), cmap="viridis"):
    ...
    # Customizing the graph
    ax.set_title(
        f"K_Means aplicado entre {variaveis[0].upper()} e {variaveis[1].upper()}",
        size=14,
        color="dimgrey",
    )
    format_spines(ax, right_border=False)
